...
- (void)scrollViewDidScroll:(UIScrollView *)sender {
    ...
    int page = listView.contentOffset.x / bounds.size.width;//通过滚动的偏移量来判断目前页面所对应的小白点
    pageControl.currentPage = page;//pagecontroll响应值的变化
    
    UIButton* rightButton = (UIButton*)self.navigationItem.rightBarButtonItem.customView;
    
    if(listView.contentOffset.x > (bounds.size.width/2))
    {
...
- (IBAction)doComment:(id)sender
{
    ...
    if(listView.contentOffset.x /bounds.size.width == 0)
        [self gotoPage:1];
    else
        [self gotoPage:0];
}
...
- (void) gotoPage:(int) pageNum
{
    ...
    [listView setContentOffset:CGPointMake(bounds.size.width * pageNum, 0)];
